...
const MapContainer = (props) => {
  ...
  useEffect(() => {
    ...
    const tourlistInfo = tourlist.current.map((info) => {
      ...
      return {
        ...
        lat: info.latitude,
        lng: info.longitude,
        road: info.roadaddress1, //roadaddress로 수정
        img: info.thumbnailpath,
        img_large: info.imgpath,
   ...
   //위에서 만든  tourlist 각각에 대해서 마커 생성
        ...
        domDesc2.classList.add('desc2');
          ...
          adr.classList.add('adr');
            ...
            toMap2.onclick = function () {
              location.href = 'https://map.kakao.com/link/map/33.355,127.126.563'//지도연결 주소 수정
            };
              let mapUrl2 = document.createTextNode('🚙길찾기');
    ...
    domContent.classList.add('wrap');// class
      ...
      domInfo.classList.add('info');
        ...
        let domTitle = document.createElement('info');//태그
        domTitle.classList.add('title');//클래스
          ...
          domClose.onclick = function () {//온클릭 펑션
            var moveLatLon = new kakao.maps.LatLng(center[props.areaCode].latMap,center[props.areaCode].lngMap)
            mapRef.current.setCenter(moveLatLon);
              if(props.areaCode===12){
              mapRef.current.setLevel(10);
      ...
      domBody.classList.add('body'); 
        ...
        domDesc.classList.add('desc');
          ...
          let moreInfoText = document.createTextNode('더보기');
          ...
          toMap.onclick = function () {
            location.href = 'https://map.kakao.com/link/map/'+elm.lat+','+elm.lng//지도연결 주소 수정
          };

            let mapUrl = document.createTextNode('🚗길찾기');
    ...
    const infowindow = new kakao.maps.InfoWindow({
      ...
      kakao.maps.event.addListener(marker, 'click', function () {
        var moveLatLon = new kakao.maps.LatLng(elm.lat+0.015, elm.lng);
        mapRef.current.setCenter(moveLatLon);
      ...
      document.getElementById(elm.title).addEventListener('click', function () {
        var moveLatLon = new kakao.maps.LatLng(elm.lat, elm.lng);
        mapRef.current.setCenter(moveLatLon);
        mapRef.current.setLevel(7);
        overlay.setMap(mapRef.current);
        });    
      ...
      let overlay2 = new kakao.maps.CustomOverlay({
        content: domContent2,
        position: new kakao.maps.LatLng((elm.lat-0.024), elm.lng-0.015),//포토카드 위치 변경
      });
  ...
  return (
    <div className="flex items-center justify-center mt-10 text-center py-30 mt-5">
      <div id="map" style={{ display: 'flex', justifyContent: 'right', bottom:"30px", width: "60vw", height: "60vh"}}>
      </div>
    </div>
  );
